...
- [ ] **Step 1: 在 model.go 的 Update 中新增 ReasoningText 事件处理分支**

- [ ] **Step 2: 实现 ingestReasoningChunk 方法**

首次推理文本时创建 EntryReasoning 摘要行，后续更新摘要行内容。

- [ ] **Step 3: 实现 renderReasoningSummary 方法**

- [ ] **Step 4: 在 transcript.go 顶部新增 reasoningDimStyle**

- [ ] **Step 5: 在 renderEntry 中处理 EntryReasoning**

- [ ] **Step 6: 实现 renderReasoningEntry 方法**
...
- [ ] **Step 1: 在 Text 事件处理中检测推理结束**

若 reasoningLineIdx >= 0，将摘要行更新为 renderReasoningSummary(true)，设置 reasoningLineIdx = -1。

- [ ] **Step 2: 在 KeyPressMsg 处理中新增 Ctrl+O 快捷键**

- [ ] **Step 3: 在 interruptTurn 中重置推理状态**

- [ ] **Step 4: 在 submit 中重置推理状态**
...
- [ ] **Step 1: 在 Model 结构体中新增流式输出状态字段**

- [ ] **Step 2: 在 New() 中初始化 toolStreamIdx = -1**
...
- [ ] **Step 1: 在 Update 中新增 ToolProgress 事件处理分支**

- [ ] **Step 2: 实现 ingestToolProgress 方法**

首次 ToolProgress 时创建流式输出块，后续追加 chunk，按换行符分割完整行 push 到 toolTail（最多 20 行）。

- [ ] **Step 3: 实现 renderToolStreamBlock 方法**
...
- [ ] **Step 1: 在 ToolResult 处理中检测流式输出结束**

若 toolStreamIdx >= 0，将流式输出块转为折叠摘要，重置流式状态字段。

- [ ] **Step 2: 实现 Drain Loop**

循环从 channel 读取最多 maxEventDrain（512）个事件，最后一次性 syncViewportContent。

- [ ] **Step 3: 定义 maxEventDrain = 512 常量**

- [ ] **Step 4: 实现 ingestDrainEvent 辅助方法**
...
- [ ] **Step 1: 在 Model 结构体中新增 Shell 输出字段**
...
- [ ] **Step 1: 在 ToolResult 处理中识别 bash 工具**

- [ ] **Step 2: 实现 1MB 截断保护**

- [ ] **Step 3: 修改 ToolResult 渲染逻辑**
...
- [ ] **Step 1: 在 KeyPressMsg 处理中新增 Ctrl+B 快捷键**

- [ ] **Step 2: 实现 toggleShellExpand 方法**

- [ ] **Step 3: 在 ToolResult 处理中将 toolCallID 写入 Raw**
...
- [ ] **Step 1: 在 NewGlamourRenderer 中启用 chroma**

在 glamour.NewTermRenderer 调用中添加 glamour.WithChromaStyle() 选项。

- [ ] **Step 2: 定义 chromaStyle 变量**
...
- [ ] **Step 1: 实现 diff 代码块检测**

在 Render 方法中，渲染完成后检测 markdown 是否包含 diff 代码块。

- [ ] **Step 2: 实现 isDiffMarkdown 检测函数**

- [ ] **Step 3: 实现 applyDiffColoring 着色函数**
...
- [ ] **Step 1: 创建 selection.go 定义 selection 结构体**

包含 startLine/startCol/endLine/endCol/active 字段和 empty() 方法。

- [ ] **Step 2: 在 Model 中新增 sel 字段**
...
- [ ] **Step 1: 在 Update 中新增 MouseMsg 选择处理**

左键按下开始选择，拖动扩展，释放保持，单击取消。

- [ ] **Step 2: 处理选择时滚轮/PgUp/PgDn 扩展选择范围**
...
- [ ] **Step 1: 在 selection.go 中实现坐标映射辅助方法**

containsLine、highlightLine、highlightRange。

- [ ] **Step 2: 在 View() 中应用选择高亮**
...
- [ ] **Step 1: 将 clipboard 依赖提升为 direct**

```powershell
cd D:\project\coding-agent; go get github.com/atotto/clipboard@v0.1.4
```

- [ ] **Step 2: 在 selection.go 中实现 extractSelectedText 方法**

- [ ] **Step 3: 修改 Ctrl+C 处理逻辑**
...
- [ ] **Step 1: 在 Model 中新增 diffMaxLines 字段**

- [ ] **Step 2: 在 toolcard.go 中实现 diff 块折叠渲染**

- [ ] **Step 3: 在 ToolResult 处理中集成 diff 折叠**
...
- [ ] **Step 1: 在 submit 中检测 /diff-fold 命令**
...
- [ ] 推理文本：ReasoningText 事件创建摘要行，Ctrl+O 切换展开/折叠
- [ ] 工具流式：ToolProgress 事件尾部截断，ToolResult 折叠摘要
- [ ] Shell 折叠：bash 输出默认折叠，Ctrl+B 切换展开
- [ ] Markdown：chroma 语法高亮，diff 代码块着色
- [ ] 文本选择：鼠标拖选高亮，Ctrl+C 复制到剪贴板
- [ ] Diff 视图：diff 块折叠，/diff-fold 命令
- [ ] 回归：go test ./... 全量通过
